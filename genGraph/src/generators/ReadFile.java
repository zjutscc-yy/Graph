...
//        writeUml(bigGraph);
    ...
    public static Graph mergeClawGraph(Graph graph, Graph pathGraph){
        if (graph.getNodes().size() == 0) {
            return pathGraph;
        }
        ...
        for (Map.Entry<ArrayList<Integer>, Integer> arrayListIntegerEntry : pathGraph.getEnvs().entrySet()) {
    ...
    public static Graph generatePathGraph(ArrayList<String> arrs,String gptPath,ArrayList<Integer> env){
...
//                br.reset();
    ...
    public void writeUml(Graph graph) throws IOException {
        ...
        //把节点和边保存到txt文件中
        File graphFile = new File("F:\\project\\SQ-MCTS\\graphView1_0.1+0.2.txt");

        FileWriter newFile = new FileWriter("graphView1_0.1+0.2.txt", true);
